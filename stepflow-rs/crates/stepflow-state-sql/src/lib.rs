...
mod tests {
    ...
    use stepflow_core::{BlobId, BlobType, FlowResult, ValueExpr, workflow::ValueRef};
    use stepflow_dtos::{StepId, StepResult};
    use stepflow_state::{
        ExecutionJournal as _, JournalEntry, JournalEvent, MetadataStore as _, SequenceNumber,
        StateStore as _,
    };
    use uuid::Uuid;
    ...
    #[tokio::test]
    async fn test_journal_checkpoint_and_get() {
        let store = SqliteStateStore::in_memory().await.unwrap();
        let run_id = Uuid::now_v7();
        ...
        let checkpoint = store.get_checkpoint(run_id).await.unwrap();
        assert!(checkpoint.is_none());

        // Set checkpoint at sequence 2
        store
            .checkpoint(run_id, SequenceNumber::new(2))
            .await
            .unwrap();

        // Verify checkpoint
        let checkpoint = store.get_checkpoint(run_id).await.unwrap();
        assert_eq!(checkpoint, Some(SequenceNumber::new(2)));

        // Update checkpoint to sequence 4
        store
            .checkpoint(run_id, SequenceNumber::new(4))
            .await
            .unwrap();

        let checkpoint = store.get_checkpoint(run_id).await.unwrap();
        assert_eq!(checkpoint, Some(SequenceNumber::new(4)));
    }

    #[tokio::test]
    async fn test_journal_compact() {
        let store = SqliteStateStore::in_memory().await.unwrap();
        let run_id = Uuid::now_v7();
        ...
        // Set checkpoint at sequence 3
        store
            .checkpoint(run_id, SequenceNumber::new(3))
            .await
            .unwrap();

        // Compact - should delete entries before checkpoint
        store.compact(run_id).await.unwrap();

        // Read all remaining entries - should only have entries >= 3
        let entries = store
            .read_from(run_id, SequenceNumber::new(0), 100)
            .await
            .unwrap();

        // Entries 0, 1, 2 should be deleted, 3 and 4 remain
        assert_eq!(entries.len(), 2);
        assert_eq!(entries[0].0.value(), 3);
        assert_eq!(entries[1].0.value(), 4);
    }
    ...
    async fn test_journal_list_active_roots() {
        ...
        // Checkpoint root1 at sequence 1
        store
            .checkpoint(root_run_id1, SequenceNumber::new(1))
            .await
            .unwrap();
        ...
        assert_eq!(
            root1_info.checkpoint_sequence,
            Some(SequenceNumber::new(1))
        );
        // Entries 0, 1 are synced (checkpoint at 1), entry 2 is not
        assert_eq!(root1_info.unsynced_count, 1);
        ...
        assert_eq!(root2_info.checkpoint_sequence, None);
    ...
    #[tokio::test]
    async fn test_journal_checkpoint_applies_to_entire_tree() {
        // Test that checkpointing applies to the entire execution tree
        let store = SqliteStateStore::in_memory().await.unwrap();

        let root_run_id = Uuid::now_v7();
        let subflow_run_id = Uuid::now_v7();

        // Add events from both parent and subflow
        for i in 0..3 {
            let entry = JournalEntry::new(
                root_run_id,
                root_run_id,
        ...
        // Checkpoint at sequence 3 (covers first 4 entries: 0, 1, 2, 3)
        store
            .checkpoint(root_run_id, SequenceNumber::new(3))
            .await
            .unwrap();

        // Compact should remove entries 0, 1, 2 (before checkpoint)
        store.compact(root_run_id).await.unwrap();

        // Should have 3 entries left: seq 3, 4, 5
        let remaining = store
            .read_from(root_run_id, SequenceNumber::new(0), usize::MAX)
            .await
            .unwrap();
        assert_eq!(remaining.len(), 3);
        assert_eq!(remaining[0].0.value(), 3);
        assert_eq!(remaining[1].0.value(), 4);
        assert_eq!(remaining[2].0.value(), 5);

        // Both parent and subflow events should be in the remaining entries
        let has_parent = remaining.iter().any(|(_, e)| e.run_id == root_run_id);
        let has_subflow = remaining.iter().any(|(_, e)| e.run_id == subflow_run_id);
        assert!(has_parent);
        assert!(has_subflow);
    }
