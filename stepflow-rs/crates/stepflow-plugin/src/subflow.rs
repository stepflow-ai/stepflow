...
pub struct SubflowRequest {
    ...
    /// Item index within the parent run that submitted this sub-flow.
    pub item_index: u32,
    /// Step index within the parent run that submitted this sub-flow.
    pub step_index: usize,
    /// Caller-provided key for subflow deduplication during recovery.
    ///
    /// Combined with `(parent_run_id, item_index, step_index)`, this uniquely
    /// identifies a subflow submission. When a step re-executes after recovery,
    /// the same key allows matching the new submission to the pre-crash subflow.
...
pub struct SubflowSubmitter {
    ...
    root_run_id: Uuid,
    /// Item index within the parent run (set by `for_task()`).
    item_index: u32,
    /// Step index within the parent run (set by `for_task()`).
    step_index: usize,
}
...
impl SubflowSubmitter {
    ...
    /// Create a submitter scoped to a specific task (run + item + step).
    ///
    /// This sets the `parent_run_id`, `item_index`, and `step_index` so that
    /// subflow submissions carry full parent task context. This context is used
    /// for subflow deduplication during recovery.
    pub fn for_task(&self, run_id: Uuid, item_index: u32, step_index: usize) -> Self {
        Self {
            tx: self.tx.clone(),
            parent_run_id: run_id,
            root_run_id: self.root_run_id,
    ...
    ///
    /// The `subflow_key` is a caller-provided UUID that uniquely identifies this
    /// submission within the scope of `(parent_run_id, item_index, step_index)`.
    /// It enables deduplication during recovery: when a step re-executes after a
    /// crash, the same key matches the new submission to the pre-crash subflow.
    ///
...
mod tests {
    ...
    async fn test_subflow_submitter_basic() {
        ...
        let subflow_key = Uuid::new_v5(&parent_run_id, &0u64.to_le_bytes());

        // Spawn a task to receive the request and acknowledge it
        let receiver_task = tokio::spawn(async move {
            let request = rx.recv().await.unwrap();
            let run_id = request.run_id;
            assert_eq!(request.subflow_key, subflow_key);
    ...
    #[tokio::test]
    async fn test_subflow_submitter_for_task() {
        let parent_run_id = Uuid::now_v7();
        let root_run_id = Uuid::now_v7();
        let (tx, _rx) = mpsc::channel(10);
        let submitter = SubflowSubmitter::new(tx, parent_run_id, root_run_id);

        let task_submitter = submitter.for_task(parent_run_id, 2, 5);
        assert_eq!(task_submitter.parent_run_id(), parent_run_id);
        assert_eq!(task_submitter.root_run_id(), root_run_id);
        assert_eq!(task_submitter.item_index, 2);
        assert_eq!(task_submitter.step_index, 5);
    }
