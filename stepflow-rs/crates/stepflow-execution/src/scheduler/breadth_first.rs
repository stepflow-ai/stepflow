...
impl Ord for BreadthFirstTask {
    fn cmp(&self, other: &Self) -> std::cmp::Ordering {
        // Reverse ordering: smaller (step_index, item_index, run_id) should come first
        // BinaryHeap is max-heap, so we reverse the comparison
        // Note: run_id is compared last so tasks from the same step/item are grouped,
        // regardless of which run they belong to (this helps batch similar work)
        (other.0.step_index, other.0.item_index, other.0.run_id).cmp(&(
            self.0.step_index,
            self.0.item_index,
            self.0.run_id,
        ))
...
mod tests {
    ...
    fn test_breadth_first_ordering() {
        ...
        assert_eq!(tasks[0], Task::new(run_id, 0, 0)); // step 0, item 0
        assert_eq!(tasks[1], Task::new(run_id, 1, 0)); // step 0, item 1
        assert_eq!(tasks[2], Task::new(run_id, 0, 1)); // step 1, item 0
        assert_eq!(tasks[3], Task::new(run_id, 1, 1)); // step 1, item 1
    }
