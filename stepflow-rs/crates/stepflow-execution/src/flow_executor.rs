...
use stepflow_state::{
    CreateRunParams, ExecutionJournalExt as _, JournalEntry, JournalEvent, StateStore,
    StateStoreExt as _, StateWriteOperation,
};
use uuid::Uuid;
...
impl FlowExecutor {
    ...
    /// Write a journal entry if journalling is enabled.
    ///
    /// This is a fire-and-forget operation - journal write failures are logged
    /// but don't halt execution. The journal is for recovery, not for correctness.
    fn write_journal(&self, run_id: Uuid, event: JournalEvent) {
        if let Some(journal) = self.env.execution_journal() {
            ...
            tokio::spawn(async move {
                ...
                if let Err(e) = journal.append(entry).await {
                    log::warn!("Failed to write journal entry for run {}: {:?}", run_id, e);
                }
            });
        }
    }
...
/// Builder for creating a FlowExecutor from a RunState.
///
/// The caller is responsible for creating the RunState (either new or recovered
/// from journal replay) before constructing the executor. This provides a uniform
/// API for both new runs and recovery.
///
/// # Example (new run)
///
/// ```ignore
...
impl FlowExecutorBuilder {
    ...
    pub fn new(env: Arc<stepflow_plugin::StepflowEnvironment>, run_state: RunState) -> Self {
        Self {
            env,
            run_state,
            scheduler: None,
