...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Phase 2: Configure prost with serde attributes via tonic-rest
    let mut config = prost_build::Config::new();
    config.file_descriptor_set_path(&descriptor_path);

    // Map google.protobuf WKT types to prost-wkt-types (serde-enabled).
    // Note: .google.protobuf.Empty is already mapped by tonic-prost-build.
    ...
    // Phase 3: Compile protos with tonic-prost-build (gRPC server/client codegen)
    // using the pre-configured prost config
    tonic_prost_build::configure()
        .build_server(true)
        .build_client(true)
        .compile_with_config(config, &proto_file_refs, include_dirs)?;

    // Phase 4: Generate REST routes from google.api.http annotations
    ...
    // Phase 5: Generate build-time metadata (git commit, dirty, tags)
