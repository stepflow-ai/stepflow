...
#[tokio::test]
async fn test_debug_api_eval_walkthrough() {
    // This test demonstrates the "eval" workflow where each step evaluation
    // automatically runs dependencies. This works across HTTP requests because
    // completed step results are persisted to the state store.
    init_test_logging();
    ...
    let workflow = FlowBuilder::new()
        .name("debug_eval_walkthrough_test")
        .description("Test workflow for debug eval walkthrough")
        .steps(vec![
            StepBuilder::new("step1")
                .component("/mock/one_output")
                .input_json(json!({"input": "debug_step"}))
    ...
    let eval_request = Request::builder()
        .uri(format!("/api/v1/runs/{run_id}/debug/eval"))
        .method("POST")
        .header("content-type", "application/json")
        .body(Body::from(
            serde_json::to_string(&json!({
    ...
    let response = app.clone().oneshot(eval_request).await.unwrap();
    assert_eq!(response.status(), StatusCode::OK);
    let body = axum::body::to_bytes(response.into_body(), usize::MAX)
        .await
        .unwrap();
...
#[tokio::test]
async fn test_debug_eval_endpoint() {
    init_test_logging();
    ...
    // Create workflow with dependent steps
    let workflow = FlowBuilder::new()
        .name("debug_eval_test")
        .description("Test workflow for debug eval endpoint")
        .steps(vec![
            StepBuilder::new("step1")
                .component("/mock/one_output")
                .input_json(json!({"input": "debug_step"}))
    ...
    // Use eval to get step2 result (should auto-run step1 first)
    let eval_request = Request::builder()
        .uri(format!("/api/v1/runs/{run_id}/debug/eval"))
        .method("POST")
        .header("content-type", "application/json")
