...
use crate::{
    values::{ValueRef, new_expr::Expr},
    workflow::PathPart,
};
use serde::Deserialize;
use serde_json::Value;
use trie_rs::{
    inc_search::{Answer, IncSearch, Position},
...
fn parse_value_template(template: Value) -> Result<ValueTemplateRepr, ParseError> {
    fn recurse<'a>(
        ...
        match v {
            ...
            Value::Object(o) if o.contains_key("$from") => match Expr::deserialize(v.clone()) {
                Ok(expr) => {
...
mod tests {
    ...
    fn test_object_with_step_references() {
        ...
        assert_eq!(
            template.exprs(),
            &[Expr::step_output("foo"), Expr::step_output("bar")]
        );
        assert_eq!(
