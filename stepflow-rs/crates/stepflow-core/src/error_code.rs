...
impl ErrorCode {
    ...
    //
    // Structural/SDK errors in the worker (component server) process
    // itself. Not in user code. Retrying won't fix these.
    // Predefined: -32000 to -32010. User-defined: -32011 to -32099.

    /// Generic worker error.
    pub const WORKER_ERROR: i64 = -32000;
    /// Requested component does not exist on the server.
    pub const COMPONENT_NOT_FOUND: i64 = -32001;
    /// Worker (component server) has not been initialized.
    pub const WORKER_NOT_INITIALIZED: i64 = -32002;
    ...
    pub const WORKER_DEPENDENCY_ERROR: i64 = -32007;
    /// Invalid or incompatible worker configuration.
    pub const WORKER_CONFIGURATION_ERROR: i64 = -32008;
...
impl schemars::JsonSchema for ErrorCode {
    ...
    fn json_schema(_: &mut schemars::SchemaGenerator) -> schemars::Schema {
        schemars::json_schema!({
            ...
            "x-enum-varnames": [
                "PARSE_ERROR", "JSON_RPC_INVALID_REQUEST", "METHOD_NOT_FOUND", "INVALID_PARAMS", "JSON_RPC_INTERNAL_ERROR",
                "WORKER_ERROR", "COMPONENT_NOT_FOUND", "WORKER_NOT_INITIALIZED", "INVALID_INPUT_SCHEMA", "INVALID_VALUE", "NOT_FOUND", "PROTOCOL_VERSION_MISMATCH", "WORKER_DEPENDENCY_ERROR", "WORKER_CONFIGURATION_ERROR",
...
mod tests {
    ...
    fn test_error_code_constants_match_ranges() {
        ...
        assert!(
            ErrorCode::JSON_RPC_INVALID_REQUEST >= -32700
                && ErrorCode::JSON_RPC_INVALID_REQUEST <= -32600
        );

        // Worker errors: -32000 to -32099
        assert!(ErrorCode::WORKER_ERROR >= -32099 && ErrorCode::WORKER_ERROR <= -32000);
        ...
        assert!(
            ErrorCode::WORKER_DEPENDENCY_ERROR >= -32099
                && ErrorCode::WORKER_DEPENDENCY_ERROR <= -32000
        );
        assert!(
            ErrorCode::WORKER_CONFIGURATION_ERROR >= -32099
                && ErrorCode::WORKER_CONFIGURATION_ERROR <= -32000
        );
    ...
    fn test_schema_enum_matches_constants() {
        ...
        assert!(codes.contains(&ErrorCode::WORKER_DEPENDENCY_ERROR));
        assert!(codes.contains(&ErrorCode::WORKER_CONFIGURATION_ERROR));
    }
}
