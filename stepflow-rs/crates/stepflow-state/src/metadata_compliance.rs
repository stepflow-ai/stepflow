...
use crate::{CreateRunParams, MetadataStore};
...
impl MetadataComplianceTests {
    ...
    pub async fn run_all<M: MetadataStore>(store: &M) {
        // Blob tests
    ...
    pub async fn test_blob_round_trip<M: MetadataStore>(store: &M) {
    ...
    pub async fn test_blob_deduplication<M: MetadataStore>(store: &M) {
    ...
    pub async fn test_blob_types<M: MetadataStore>(store: &M) {
    ...
    pub async fn test_flow_round_trip<M: MetadataStore>(store: &M) {
    ...
    pub async fn test_get_nonexistent_blob<M: MetadataStore>(store: &M) {
    ...
    pub async fn test_create_run<M: MetadataStore>(store: &M) {
        let flow = Arc::new(create_test_flow());
        let flow_id = store
            .store_flow(flow)
            .await
        ...
        assert_eq!(run.inputs.len(), 1, "Item count should be 1");
    }

    /// Test that create_run is idempotent.
    ///
    /// Contract: Calling create_run twice with the same run_id is a no-op.
    pub async fn test_create_run_idempotent<M: MetadataStore>(store: &M) {
        let flow = Arc::new(create_test_flow());
        let flow_id = store
            .store_flow(flow)
            .await
    ...
    pub async fn test_get_nonexistent_run<M: MetadataStore>(store: &M) {
    ...
    pub async fn test_run_status_transitions<M: MetadataStore>(store: &M) {
        let flow = Arc::new(create_test_flow());
    ...
    pub async fn test_list_runs_empty<M: MetadataStore>(store: &M) {
    ...
    pub async fn test_list_runs_with_status_filter<M: MetadataStore>(store: &M) {
        use stepflow_dtos::RunFilters;
    ...
    pub async fn test_list_runs_with_limit<M: MetadataStore>(store: &M) {
        use stepflow_dtos::RunFilters;
    ...
    pub async fn test_subflow_hierarchy<M: MetadataStore>(store: &M) {
        let flow = Arc::new(create_test_flow());
        let flow_id = store.store_flow(flow).await.unwrap();
    ...
    pub async fn test_list_runs_by_root<M: MetadataStore>(store: &M) {
        use stepflow_dtos::RunFilters;
    ...
    pub async fn test_list_runs_by_parent<M: MetadataStore>(store: &M) {
        use stepflow_dtos::RunFilters;
    ...
    pub async fn test_multilevel_hierarchy<M: MetadataStore>(store: &M) {
        use stepflow_dtos::RunFilters;
    ...
    pub async fn test_list_runs_roots_only<M: MetadataStore>(store: &M) {
        use stepflow_dtos::RunFilters;
    ...
    pub async fn test_item_results_single_item<M: MetadataStore>(store: &M) {
        let flow = Arc::new(create_test_flow());
        let flow_id = store.store_flow(flow).await.unwrap();
        ...
        store
            .record_item_result(run_id, 0, result.clone())
            .await
    ...
    pub async fn test_item_results_multiple_items<M: MetadataStore>(store: &M) {
        let flow = Arc::new(create_test_flow());
        let flow_id = store.store_flow(flow).await.unwrap();
    ...
    pub async fn test_item_results_ordering_by_index<M: MetadataStore>(store: &M) {
        let flow = Arc::new(create_test_flow());
        let flow_id = store.store_flow(flow).await.unwrap();
    ...
    pub async fn test_list_pending_runs<M: MetadataStore>(store: &M) {
        let flow = Arc::new(create_test_flow());
        let flow_id = store.store_flow(flow).await.unwrap();
    ...
    pub async fn test_list_pending_runs_excludes_terminal<M: MetadataStore>(store: &M) {
        let flow = Arc::new(create_test_flow());
        let flow_id = store.store_flow(flow).await.unwrap();
