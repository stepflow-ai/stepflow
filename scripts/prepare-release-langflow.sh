...
Prepare a Langflow integration release by bumping version and generating changelog.
...
if [[ ! -d "integrations/langflow" ]] || [[ ! -f "integrations/langflow/pyproject.toml" ]]; then
    echo -e "${RED}Error: Must be run from project root directory${NC}" >&2
    echo "Expected to find integrations/langflow/pyproject.toml" >&2
    exit 1
fi

# Change to langflow integration directory for operations
...
echo -e "${BLUE}Generating changelog (Langflow integration changes only)${NC}"
if [[ -n "$TAG_MESSAGE" ]]; then
    echo -e "${BLUE}Including custom message: ${GREEN}$TAG_MESSAGE${NC}"
    git-cliff --config cliff.toml --tag "stepflow-langflow-$NEW_VERSION" -u --prepend "$CHANGELOG_PATH" --with-tag-message "$TAG_MESSAGE"
else
    git-cliff --config cliff.toml --tag "stepflow-langflow-$NEW_VERSION" -u --prepend "$CHANGELOG_PATH"
fi
...
RELEASE_BRANCH="release/stepflow-langflow-v$NEW_VERSION"
...
git commit -m "chore: release stepflow-langflow-integration v$NEW_VERSION
...
# Create PR
PR_BODY="## Release stepflow-langflow-integration v$NEW_VERSION

This PR prepares the release of stepflow-langflow-integration v$NEW_VERSION.
...
PR_URL=$(gh pr create \
    --title "chore: release stepflow-langflow-integration v$NEW_VERSION" \
    --body "$PR_BODY" \
    --label "release" \
    --label "release:stepflow-langflow")
