...
Prepare a Docling Proto Step Worker release by bumping version and generating changelog.
...
if [[ ! -d "integrations/docling-proto-step-worker" ]] || [[ ! -f "integrations/docling-proto-step-worker/pyproject.toml" ]]; then
    echo -e "${RED}Error: Must be run from project root directory${NC}" >&2
    echo "Expected to find integrations/docling-proto-step-worker/pyproject.toml" >&2
    exit 1
fi

# Change to docling-proto-step-worker directory for operations
...
echo -e "${BLUE}Generating changelog (Docling Proto Step Worker changes only)${NC}"
if [[ -n "$TAG_MESSAGE" ]]; then
    echo -e "${BLUE}Including custom message: ${GREEN}$TAG_MESSAGE${NC}"
    git-cliff --config cliff.toml --tag "stepflow-docling-$NEW_VERSION" -u --prepend "$CHANGELOG_PATH" --with-tag-message "$TAG_MESSAGE"
else
    git-cliff --config cliff.toml --tag "stepflow-docling-$NEW_VERSION" -u --prepend "$CHANGELOG_PATH"
fi
...
RELEASE_BRANCH="release/stepflow-docling-v$NEW_VERSION"
...
git commit -m "chore: release docling-proto-step-worker v$NEW_VERSION
...
# Create PR
PR_BODY="## Release docling-proto-step-worker v$NEW_VERSION

This PR prepares the release of docling-proto-step-worker v$NEW_VERSION.
...
2. Merge this PR
3. Docker images will be automatically built and pushed to GHCR"

PR_URL=$(gh pr create \
    --title "chore: release docling-proto-step-worker v$NEW_VERSION" \
    --body "$PR_BODY" \
    --label "release" \
    --label "release:docling")
...
echo "3. Docker images will be automatically built and pushed to GHCR"
