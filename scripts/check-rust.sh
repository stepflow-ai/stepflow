...
# Note: Isolation proxy integration tests (subprocess + Firecracker) run in
# a dedicated CI job (firecracker-checks) that sets up KVM, Firecracker, and
# a rootfs. See .github/actions/firecracker-checks/action.yml.
...
run_check "Documentation" env RUSTDOCFLAGS="-D warnings" cargo doc --all --no-deps || true
