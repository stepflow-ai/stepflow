...
class TestWorkflowConversion:
    ...
    @pytest.mark.parametrize(
        "workflow",
        pytest_parametrize_workflows(get_test_registry().get_conversion_test_cases()),
    )
    def test_workflow_conversion(
        self, workflow: TestWorkflow, registry, converter: LangflowConverter
    ):
        ...
        try:
            ...
            if workflow.conversion.should_succeed:
                pytest.fail(
                    f"Required Langflow file not found: {workflow.langflow_file}"
                )
            else:
        ...
        if workflow.conversion.should_succeed:
            ...
            self._assert_conversion_failure(
                converter, langflow_data, workflow.conversion
            )
...
class TestConversionErrorHandling:
    ...
    def test_malformed_node(self, converter: LangflowConverter):
        ...
        invalid_data = {"data": {"nodes": [{"invalid": "node"}], "edges": []}}
        ...
        circular_data = {
            "data": {
                "nodes": [
                    {
                        "id": "node1",
                        "data": {"type": "TestComponent", "node": {"template": {}}},
                        "type": "genericNode",
                    },
                    {
                        "id": "node2",
