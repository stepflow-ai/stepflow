...
class TestWorkflowExecution:
        ...
        if workflow.execution.should_succeed:
            if not success:
                ...
                if self._is_component_availability_error(stderr):
                    pytest.skip(f"Required components not available in test environment: {stderr}")
                else:
            ...
            assert not success, f"Workflow execution should have failed but succeeded: {result_data}"
    ...
    def _assert_execution_success(self, result_data: Any, workflow: TestWorkflow, input_data: Dict[str, Any]):
        ...
        assert isinstance(result_data, dict), f"Result should be dict for {workflow.name}, got {type(result_data)}"
        ...
        if expectations.result_structure:
            ...
            self._validate_result_structure(validation_data, expectations.result_structure, workflow.name)

        # Workflow-specific validation using registry-defined validators
        if expectations.result_validator:
            expectations.result_validator.validate(result_data, input_data, workflow.name)

    def _validate_result_structure(self, result_data: Dict[str, Any], expected_structure: Dict[str, Any], workflow_name: str):
        """Validate result matches expected structure schema."""
        
        # Check type
        expected_type = expected_structure.get("type", "object")
        if expected_type == "object":
            assert isinstance(result_data, dict), f"Expected object type for {workflow_name}"
        elif expected_type == "array":
            assert isinstance(result_data, list), f"Expected array type for {workflow_name}"
        elif expected_type == "string":
            assert isinstance(result_data, str), f"Expected string type for {workflow_name}"
        ...
        if expected_type == "object" and "required_fields" in expected_structure:
            for field in expected_structure["required_fields"]:
                assert field in result_data, f"Required field '{field}' missing in result for {workflow_name}"
            ...
            for field, field_type in expected_structure["field_types"].items():
                if field in result_data:
                    ...
                    if field_type == "string":
                        assert isinstance(actual_value, str), f"Field '{field}' should be string in {workflow_name}"
                    elif field_type == "number":
                        assert isinstance(actual_value, (int, float)), f"Field '{field}' should be number in {workflow_name}"
                    elif field_type == "boolean":
                        assert isinstance(actual_value, bool), f"Field '{field}' should be boolean in {workflow_name}"
...
"""
        ...
        assert result_data is not None, "Should have result data even for failed execution"
        ...
        input_error_indicators = ["resolve", "value", "undefined", "field", "required_field"]
        has_input_error = any(indicator in error_message for indicator in input_error_indicators)
        assert has_input_error, f"Error should indicate missing input issue: {error_message}"
...
"""
        ...
        try:
            ...
            assert success, "Simple workflow should complete successfully if it doesn't timeout"
            
        except Exception as e:
            # Should be timeout exception
            error_message = str(e).lower()
            timeout_indicators = ["timeout", "timed out", "expired"]
            has_timeout_indication = any(indicator in error_message for indicator in timeout_indicators)
...
class TestExecutionPerformance:
    ...
    def test_execution_performance_characteristics(
        ...
        if performance_results:
            ...
            for result in sorted(performance_results, key=lambda x: x["execution_time"], reverse=True):
                status = '✅' if result['success'] else '❌'
                print(f"  {result['workflow']:20} | {result['nodes']:2} nodes | {result['execution_time']:6.3f}s | {result['result_size']:4}B | {status}")
        else:
