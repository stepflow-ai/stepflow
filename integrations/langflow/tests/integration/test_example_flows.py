...
def test_simple_agent(converter, stepflow_runner):
    ...
    with StepflowConfigBuilder() as config:
        ...
        result = execute_complete_flow_lifecycle(
            ...
            input_data={
                "message": f"Calculate this exact expression: {complex_expression}. Show your work and give me the precise numerical result.",
        ...
        # Should return a Langflow Message with text content containing the calculation result
        message_result = result["result"]
        assert isinstance(message_result, dict)
        assert "__langflow_type__" in message_result
        assert "text" in message_result
        ...
        assert result_found, f"Expected result (12158) not found in response: {response_text}"
        
        # CRITICAL: Verify that tools were actually used by checking database message history
        ...
        assert tool_usage_verified, f"No evidence of calculator tool usage found in database. Expression: {complex_expression}"
