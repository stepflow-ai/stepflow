...
class CachedStepflowContext:
    ...
    async def get_blob(self, blob_id: str) -> Any:
        ...
        if blob_id in self.cached_blobs:
            ...
            print(f"WARNING: Blob {blob_id} not found in cache, attempting direct fetch")
        ...
        async def udf_executor(input_data: Dict[str, Any], context: StepflowContext) -> Dict[str, Any]:
            ...
            for key, value in input_data.items():
                if isinstance(value, dict) and '$from' in value:
                    ...
                    print(f"DEBUG CACHE: {key} = {type(value).__name__}: {str(value)[:100]}")
            ...
            for blob_id in blob_ids_to_cache:
                try:
                    ...
                    print(f"DEBUG: Successfully cached blob {blob_id} with keys: {list(blob_data.keys()) if isinstance(blob_data, dict) else type(blob_data)}")
