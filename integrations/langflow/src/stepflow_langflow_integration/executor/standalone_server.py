...
async def _pre_resolve_all_blobs(input_data: Dict[str, Any], context: StepflowContext) -> Dict[str, Any]:
    ...
    for key, value in input_data.items():
        ...
        elif key.startswith("tool_blob_") and isinstance(value, dict) and "blob_id" in value:
    ...
    print(f"DEBUG: Found {len(blob_ids_to_resolve)} blob IDs to resolve: {blob_ids_to_resolve}")
    with open('/tmp/udf_debug.log', 'a') as f:
        f.write(f"DEBUG: Found {len(blob_ids_to_resolve)} blob IDs to resolve: {blob_ids_to_resolve}\n")
    ...
    with open('/tmp/udf_debug.log', 'a') as f:
        f.write(f"DEBUG: Pre-resolution complete. Resolved {len(resolved_blobs)} blobs\n")
...
async def langflow_file(input_data: Dict[str, Any], context: StepflowContext) -> Dict[str, Any]:
...
async def langflow_memory(input_data: Dict[str, Any], context: StepflowContext) -> Dict[str, Any]:
...
async def langflow_url(input_data: Dict[str, Any], context: StepflowContext) -> Dict[str, Any]:
...
async def langflow_language_model(input_data: Dict[str, Any], context: StepflowContext) -> Dict[str, Any]:
    ...
    if isinstance(input_value, str):
        ...
        user_message = input_value.get("text", input_value.get("content", str(input_value)))
    else:
...
async def langflow_astradb(input_data: Dict[str, Any], context: StepflowContext) -> Dict[str, Any]:
    ...
    for key, value in input_data.items():
        if key.startswith("_embedding_config_"):
            ...
            if isinstance(value, dict) and value.get("component_type") == "OpenAIEmbeddings":
                embedding_config = value.get("config", {})
                embedding_field = key.replace("_embedding_config_", "")
                print(f"DEBUG AstraDB: Extracted {embedding_field} config: {embedding_config}")
                break
