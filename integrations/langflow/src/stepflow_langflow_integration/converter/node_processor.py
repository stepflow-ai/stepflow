...
class NodeProcessor:
    ...
    def _generate_step_id(self, node_id: str, component_type: str) -> str:
        ...
        """
        # Remove common Langflow ID suffixes like "-123abc"
        base_id = node_id
        if "-" in base_id:
            parts = base_id.split("-")
        ...
        # If base_id is just the component type, use it directly
        if base_id.lower() == component_type.lower():
            return f"langflow_{component_type.lower()}"
        
        # Otherwise combine them
        return f"langflow_{base_id.lower()}"
