...
import { useRuns } from '@/lib/hooks/use-flow-api'
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center space-y-2">
          <p className="text-red-600">Failed to load runs</p>
          ...
          <Card>
            <CardContent className="flex items-center justify-center h-32">
              <div className="text-center space-y-2">
                ...
                <p className="text-sm text-muted-foreground">Execute a flow to see results here</p>
              </div>
            </CardContent>
          </Card>
        ) : (
          runs?.map((execution) => (
            <Card key={execution.runId}>
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-2">
                    {getStatusIcon(execution.status)}
                    <CardTitle className="text-lg">{execution.flowName || 'Ad-hoc Flow'}</CardTitle>
                    {getStatusBadge(execution.status)}
                  </div>
                  <div className="flex items-center space-x-2">
                    <Button variant="ghost" size="sm" asChild>
                      <Link href={`/runs/${execution.runId}`}>
                        View Details
                      </Link>
                    </Button>
