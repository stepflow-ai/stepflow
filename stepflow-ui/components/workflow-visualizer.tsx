...
function StepNode({ data }: StepNodeProps) {
  ...
  // Get input fields from dependencies for this step
...
// Removed custom ElkEdge component - now using React Flow's built-in smoothstep edges for better performance and appearance

// ELK-based automatic layout for node positioning only
const elk = new ELK()
...
async function getLayoutedElements(nodes: Node[], edges: Edge[]) {
  ...
  console.log('Setting up ELK with nodes:', nodes.length, 'edges:', edges.length)
  console.log('Node types:', nodes.map(n => ({ id: n.id, type: n.type, inputFields: n.data?.inputFields })))

  // Convert React Flow nodes to ELK format - only for positioning, not edge routing
  const elkNodes = nodes.map((node) => {
    // Calculate height based on content for step nodes
    ...
    // Smaller size for workflow input/output nodes
  ...
  // Simple ELK edges - just for layout, React Flow will handle routing
  ...
  nodes.forEach((node) => {
    ...
    if (elkNode) {
      ...
      console.log(`Node ${node.id} moved from (${oldPosition.x}, ${oldPosition.y}) to (${node.position.x}, ${node.position.y})`)
    }
  })

  // Edges already have the correct React Flow configuration from calculateLayout
  // No need to modify them here - let React Flow handle the routing

  return { nodes, edges }
}

// Simplified layout algorithm using ELK
async function calculateLayout(steps: StepData[], dependencies?: StepDependency[], workflow?: any): Promise<{ nodes: Node[], edges: Edge[] }> {
  ...
  // Process dependencies to find workflow output steps and track skip fields
  ...
  if (hasWorkflowInputDeps) {
    nodes.push({
      ...
      position: { x: 0, y: 0 }, // ELK will set positions
      data: {
        id: 'workflow-input',
        name: 'Workflow Input',
        inputFields: [] // No input fields for workflow input node
      } as unknown as Record<string, unknown>
    })
  }
  ...
  if (hasWorkflowOutputDeps) {
    nodes.push({
      ...
      data: {
        ...
        inputFields: [] // No input fields for workflow output node (it uses input-result)
      } as unknown as Record<string, unknown>
    })
  }
  ...
  if (dependencies && dependencies.length > 0) {
    dependencies.forEach(dep => {
      ...
      if (dep.step_index > 1000000) { // Large number indicates usize::MAX - workflow output
        const sourceStepId = stepIndexMap.get(dep.depends_on_step_index)
        if (sourceStepId) {
          edges.push({
      ...
      if (sourceStepId && targetStepId) {
        // Determine edge styling based on skip action
        ...
        if (isSkipCondition) {
          // Skip conditions get orange color
          edgeStyle = { strokeWidth: 2, stroke: '#f97316' }
        } else if (isOptional) {
          // Optional dependencies get dashed lines
...
const edgeTypes = {
  // Using React Flow's built-in edge types for better performance and smoother curves
}
...
export function WorkflowVisualizer({
  ...
  useEffect(() => {
    ...
    const runLayout = async () => {
      setIsLayouting(true)
      try {
        const { nodes, edges } = await calculateLayout(steps, dependencies, workflow)
        ...
        console.log('Setting layouted nodes:', nodesWithCallbacks.length, nodesWithCallbacks)
        console.log('Setting layouted edges:', edges.length, edges)
    ...
    runLayout()
  }, [steps, dependencies, workflow, isDebugMode, handleStepClick, onStepExecute])
  ...
  useEffect(() => {
    if (layoutedNodes.length > 0) {
      console.log('Updating React Flow nodes:', layoutedNodes.length)
  ...
  useEffect(() => {
    if (layoutedEdges.length > 0) {
      console.log('Updating React Flow edges:', layoutedEdges.length)
      setEdges(layoutedEdges)
    }
  }, [layoutedEdges, setEdges])

  console.log('Current React Flow state - nodes:', nodes.length, 'edges:', edges.length)
