...
async def iterate(input: IterateInput, context: StepflowContext) -> IterateOutput:
    ...
    while iterations < input.max_iterations:
        ...
        # Result doesn't have expected structure
            ...
            from stepflow_sdk.exceptions import StepflowValueError
...
if __name__ == "__main__":
    ...
    print(
        f"Registered components: {list(server._server.get_components().keys())}",
        file=sys.stderr,
    )
    ...
    print(f"Server initialized: {server._server.is_initialized()}", file=sys.stderr)
