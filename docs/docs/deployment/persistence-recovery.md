...
state_store:
  type: sqlite
  database_url: "sqlite:/path/to/stepflow.db?mode=rwc"
  auto_migrate: true
  max_connections: 10
```
...
state_store:
  type: sqlite
  database_url: "sqlite:stepflow.db?mode=rwc"
  auto_migrate: true
```
...
| `check_interval_secs` | `30` | Seconds between orphan check attempts |
| `max_startup_recovery` | `100` | Maximum runs to recover when orchestrator starts |
| `max_claims_per_check` | `10` | Maximum orphans to claim in each periodic check |
...
state_store:
  type: sqlite
  database_url: "sqlite:/var/lib/stepflow/state.db?mode=rwc"
  auto_migrate: true
```
