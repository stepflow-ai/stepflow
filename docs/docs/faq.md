...
- **Open protocol, any language.** Workers pull tasks from [pull-based task queues](./protocol/index.md) and return results via gRPC. You can build workers in Python (using the [SDK](./workers/custom-components.md)), implement the protocol directly in any language, or use [MCP servers](./components/mcp-tools.md) as components with zero wrapping code.
