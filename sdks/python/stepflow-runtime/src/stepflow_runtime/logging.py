...
@dataclass
class LogConfig:
    """Configuration for stepflow-server logging.
    ...
    def to_cli_args(self) -> list[str]:
        """Convert config to CLI arguments for stepflow-server."""
        args = [
            "--log-level",
            self.level,
            "--log-format",
        ...
        if self.file_path:
            args.extend(["--log-destination", "file", "--log-file", str(self.file_path)])
        elif self.otlp_endpoint:
            args.extend(["--log-destination", "otlp", "--otlp-endpoint", self.otlp_endpoint])
        else:
