...
def _convert_api_result_to_flow_result(
    ...
    # Handle raw dict format: {outcome: "success"|"failure"|"skipped", ...}
...
class StepflowRuntime:
    ...
    This class manages a stepflow-worker subprocess, providing:
    ...
    def start(
        ...
        *,
        ...
        Raises:
            ...
            FileNotFoundError: If the stepflow-worker binary is not found.
    ...
    def stop(self, timeout: float = 5.0) -> None:
        ...
        if self._process is not None and self._process.poll() is None:
            logger.info("Stopping stepflow-worker...")
            try:
        ...
        self._process = None
        logger.info("stepflow-worker stopped")
    ...
    def _start_process(self) -> None:
        """Start the stepflow-worker subprocess."""
        ...
        logger.info(f"Starting stepflow-worker on port {self._port}")
        ...
        try:
            ...
            raise StepflowRuntimeError(f"Failed to start stepflow-worker: {e}") from e
    ...
    def _wait_for_ready_sync(self) -> None:
        ...
        while time.monotonic() - start < self._startup_timeout:
            ...
            if self._process and self._process.poll() is not None:
                ...
                raise StepflowRuntimeError(
                    f"stepflow-worker exited during startup with code {exit_code}"
                )
            ...
            try:
                with httpx.Client(timeout=1.0) as client:
                    ...
                    if response.status_code == 200:
                        logger.info("stepflow-worker is ready")
                        return
        ...
        raise TimeoutError(
            f"stepflow-worker did not become ready within {self._startup_timeout}s"
        )
    ...
    def _handle_crash(self, exit_code: int) -> None:
        ...
        logger.error(f"stepflow-worker exited with code {exit_code}")
        ...
        if should_restart and self._restart_count < self._max_restarts:
            ...
            try:
                ...
                logger.info("stepflow-worker restarted successfully")
            except Exception as e:
                logger.error(f"Failed to restart stepflow-worker: {e}")
                self._crashed = True
        else:
            if should_restart:
                logger.error(
