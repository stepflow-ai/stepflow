...
def test_variables_schema_from_dict():
    ...
    # FlowV1 has 'variables' as a direct attribute (not nested under schemas)
...
def test_variables_schema_load_flow():
    ...
    # Variables schema should be preserved (as empty dict since Schema can't hold data)
    assert loaded_builder.variables_schema is not None
    assert loaded_builder.variables_schema == {}  # Empty dict when loaded
...
def test_variables_schema_none_by_default():
    ...
    # No variables schema should be set if not provided
