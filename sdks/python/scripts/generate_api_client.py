...
openapi-python-client, then post-processes to add generated file headers.
...
Usage:
    ...
    python scripts/generate_api_client.py check                 # Check if generated files are up-to-date
...
CONFIG_FILE = SCRIPT_DIR / "openapi-client-config.yaml"
...
def run_openapi_python_client(spec_file: Path, output_dir: Path) -> Path:
    """Run openapi-python-client to generate the client."""
    print(">>> Running openapi-python-client...")
...
def format_files(directory: Path) -> None:
    ...
    subprocess.run(
        ["uvx", "ruff@0.9.4", "check", "--fix", "--config", str(PYTHON_SDK_DIR / "pyproject.toml"), str(directory)],
        capture_output=True,
    )
...
def copy_generated_files(src: Path, dest: Path) -> None:
    ...
    # openapi-python-client generates to stepflow_api/ (flat structure)
    ...
    generated_files = ["__init__.py", "client.py", "errors.py", "types.py", "py.typed"]
...
def do_generate(spec_file: Path) -> None:
    ...
    generated = run_openapi_python_client(spec_file, _temp_dir)
...
def cmd_check(args: argparse.Namespace) -> int:
    ...
    # Generate fresh
    generated = run_openapi_python_client(STORED_SPEC, _temp_dir)
    generated_pkg = generated / "stepflow_api"
...
""",
    ...
    gen_parser = subparsers.add_parser(
        ...
        description="Regenerate client from the stored OpenAPI spec or a specific file.",
    )
    ...
    subparsers.add_parser(
        ...
        description="Verify generated client matches the stored OpenAPI spec. Exits non-zero if out of date.",
    )
