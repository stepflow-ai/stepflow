...
class ModelIf(BaseModel):
    ...
    def from_dict(cls, obj: dict[str, Any] | None) -> Self | None:
        ...
        _obj = cls.model_validate(
            {
                "$if": ValueExpr.from_dict(obj["$if"])
                if obj.get("$if") is not None
                else None,
                "then": ValueExpr.from_dict(obj["then"])
                if obj.get("then") is not None
                else None,
                "else": ValueExpr.from_dict(obj["else"])
...
ModelIf.model_rebuild(raise_errors=False)
