        ...
        *,
        ...
        def decorator(f: Callable) -> Callable:
            ...
            try:
                ...
                raise StepflowExecutionError(f"Failed to create LangChain runnable: {e}") from e
            ...
            try:
                ...
                if not isinstance(runnable, Runnable):
                    raise StepflowExecutionError(
                        f"Function {f.__name__} must return a LangChain Runnable instance, "
                        f"got {type(runnable)}"
                    )
            except ImportError:
                raise StepflowExecutionError("LangChain not available")
            ...
            async def langchain_component_executor(
                ...
                if exec_mode not in ["invoke", "batch", "stream"]:
                    raise StepflowExecutionError(
                        f"Invalid execution_mode '{exec_mode}'. Must be 'invoke', 'batch', or 'stream'"
                    )
...
def get_langchain_components(server) -> Dict[str, Dict[str, Any]]:
    ...
    return server._langchain_components.copy()
