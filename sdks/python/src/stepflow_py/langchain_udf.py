...
async def langchain_udf(input: LangChainUdfInput, context: StepflowContext) -> Any:
    ...
    if input.blob_id in _runnable_cache:
        print(
            f"Using cached LangChain runnable for blob_id: {input.blob_id}",
            file=sys.stderr,
        )
        ...
        print(
            f"Loading and deserializing LangChain runnable for blob_id: "
            f"{input.blob_id}",
            file=sys.stderr,
        )
        ...
        try:
            ...
            raise StepflowExecutionError(
                f"Failed to retrieve blob {input.blob_id}: {e}"
            ) from e

        # Validate blob structure
        if not isinstance(blob_data, dict):
            raise StepflowExecutionError(
                f"Blob {input.blob_id} must contain a dictionary"
            )
        ...
        if input_schema:
            try:
                ...
                print(
                    "jsonschema not available, skipping input validation",
                    file=sys.stderr,
                )
                ...
                raise StepflowExecutionError(
                    f"Invalid input schema: {e.message}"
                ) from e
    ...
    try:
        ...
        if output_schema:
            try:
                ...
                print(
                    "jsonschema not available, skipping output validation",
                    file=sys.stderr,
                )
            except jsonschema.ValidationError as e:
                raise StepflowExecutionError(
                ...
                raise StepflowExecutionError(
                    f"Invalid output schema: {e.message}"
                ) from e
